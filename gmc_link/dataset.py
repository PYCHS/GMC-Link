...
class ContrastiveMotionLanguageDataset(Dataset):
    """
    PyTorch Dataset for InfoNCE-based contrastive training.
    Each sample: (motion_vector, language_embedding, sentence_id)
    Negatives are implicit — they come from other samples in the batch.
    """

    def __init__(self, motion_data, language_data, sentence_ids):
        assert len(motion_data) == len(language_data) == len(sentence_ids)
        self.motion_data = motion_data
        self.language_data = language_data
    ...
    def __getitem__(self, idx):
        motion = torch.tensor(self.motion_data[idx], dtype=torch.float32)
        lang = torch.tensor(self.language_data[idx], dtype=torch.float32)
        sid = torch.tensor(self.sentence_ids[idx], dtype=torch.long)
        return motion, lang, sid


def contrastive_collate_fn(batch):
    """Stack into (B, 8) motion, (B, L_dim) language, and (B,) sentence_ids."""
    motion_batch = torch.stack([item[0] for item in batch], dim=0)
    language_batch = torch.stack([item[1] for item in batch], dim=0)
    sid_batch = torch.stack([item[2] for item in batch], dim=0)
    return motion_batch, language_batch, sid_batch


def _generate_contrastive_samples(
    track_centroids: Dict[int, Dict[int, Tuple[float, float, float, float]]],
    sentence: str,
    embedding: np.ndarray,
    sentence_id: int,
    ...
    """
    Generate positive (motion, language) samples for contrastive training.
    No explicit negatives — those come from other samples in the batch.
    Includes jittered augmentations as additional positive samples.
    """
    h, w = frame_shape
    motion_data = []
    language_data = []
    ...
    for tid, centroids in track_centroids.items():
        ...
        for i in range(len(sorted_frames)):
            ...
            # Jittered augmentation: ±2px synthetic noise (same sentence_id)
            jittered = motion_vec.copy()
            jitter_px = np.random.uniform(-2.0, 2.0, size=4)
            jittered[0] += jitter_px[0] / w * VELOCITY_SCALE
            jittered[1] += jitter_px[1] / h * VELOCITY_SCALE
            jittered[2] += jitter_px[2] / w * VELOCITY_SCALE
            jittered[3] += jitter_px[3] / h * VELOCITY_SCALE
            motion_data.append(jittered)
            language_data.append(embedding.copy())
            sentence_ids.append(sentence_id)

    return motion_data, language_data, sentence_ids


def build_contrastive_training_data(
    data_root: str,
    sequences: List[str],
    text_encoder: Any,
    frame_gap: int = 5,
    ...
    """
    Build (motion, language, sentence_id) training data for InfoNCE contrastive training.
    Uses ORB Homography for ego-motion compensation (same as BCE pipeline).
    """
    all_expressions, sentence_embeddings, all_sentences = _collect_expressions(
        data_root, sequences, text_encoder
    )

    # Build sentence → integer ID mapping
    sentence_to_id = {s: i for i, s in enumerate(all_sentences)}
    print(f"  Unique sentences: {len(sentence_to_id)}")

    print("  Initializing ORBHomographyEngine for dataset ego-motion compensation...")
    orb_engine = ORBHomographyEngine(max_features=1500)

    print("  Generating contrastive training samples (positives only)...")
    motion_data = []
    language_data = []
    sentence_ids_list = []
    ...
    for expr in all_expressions:
        ...
        m_data, l_data, sids = _generate_contrastive_samples(
            track_centroids,
            sentence,
            embedding,
            sid,
        ...
        motion_data.extend(m_data)
        language_data.extend(l_data)
        sentence_ids_list.extend(sids)

    # Report distribution
    from collections import Counter
    sid_counts = Counter(sentence_ids_list)
    print(f"  Total samples: {len(motion_data)}")
    print(f"  Sentence distribution: min={min(sid_counts.values())}, "
          f"max={max(sid_counts.values())}, "
          f"mean={sum(sid_counts.values())/len(sid_counts):.1f}")

    return motion_data, language_data, sentence_ids_list
